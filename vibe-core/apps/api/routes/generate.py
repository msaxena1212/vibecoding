    ...
    async def stream_generator():
        ...
        try:
            ...
            try:
                ...
                traceback.print_exc()
                # Continue to attempt saving what we have

            # Workflow finished, process results
            from utils.formatter import sanitize_state
            sanitized_result = sanitize_state(final_result)
            
            # Save to DB - Ensure we use active_project_id
            ...
            # Correct path for centralized project hub
            project_dir = os.path.join("..", "vibe-projects", new_project_id)
            os.makedirs(project_dir, exist_ok=True)
            for path, file_data in sanitized_result.get("files", {}).items():
                file_full_path = os.path.join(project_dir, path)
