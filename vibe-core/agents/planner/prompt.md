...
8. **CTA-Driven Architecture**: Every high-visibility Call-to-Action (CTA) or Button (especially on the Home page) MUST have a corresponding destination page planned and generated. No decorative buttons allowed.
...
- **Interactive Completeness**: If you plan a CTA like "Book Now" or "Explore Details", you MUST include a corresponding file (e.g., `booking.html`, `details.html`) in the `files` list and the `navigation_map`.
